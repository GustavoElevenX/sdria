...
export function LearningInsightCard({ insight }: { insight: LearningInsight }) {
  async function mutate(action: "approve" | "reject") {
    await fetch(`/api/learning/insights/${insight.id}/${action}`, { method: "POST" });
  ...
  return (
    <Panel className="p-4">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Button className="h-8" onClick={() => mutate("approve")}><Check size={15} /> Aprovar</Button>
        <Button className="h-8" variant="secondary" onClick={() => mutate("reject")}><X size={15} /> Rejeitar</Button>
